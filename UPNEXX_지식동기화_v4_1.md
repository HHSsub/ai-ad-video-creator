...
### ⛔ API 경로 규칙 (38번째 실수 방지)

**모든 프론트엔드 API 호출은 반드시 `API_BASE` 상수 사용:**

```javascript
// ✅ 올바른 방법
const API_BASE = import.meta.env.VITE_API_BASE_URL || '/nexxii';
fetch(`${API_BASE}/api/storyboard-init`, { ... });
fetch(`${API_BASE}/api/api-keys`, { ... });

// ❌ 절대 금지 - S3로 요청이 감
fetch('/api/storyboard-init', { ... });
fetch('/api/api-keys', { ... });
```

**이유:**
- `.env`: `VITE_API_BASE_URL=/nexxii`
- **nginx 설정**: `/nexxii/api/` → Node.js 서버, `/api/` → S3 버킷
- `/api/`로 시작하면 S3 버킷으로 라우팅되어 404 또는 HTML 에러 반환

**작업 전 필수 확인:**
1. CRITICAL_CONFIG.md 읽기
2. 새 컴포넌트/API 호출 추가 시 `API_BASE` 상수 먼저 선언
3. 모든 fetch 호출에 `${API_BASE}` prefix 포함 확인
...
> 
> 1. **작업 시작 전**: CRITICAL_CONFIG.md + 지식동기화 최상단 규칙 + 현재 진행 상황 확인
> 2. **작업 중**: 기존 코드 패턴 참조 (최소 3개), 상호의존성 검증, 변수/상수 사용처 즉시 확인
> 3. **작업 완료 후**: 작업 히스토리에 기록 (prepend 방식)
> 4. 구현 완료 판단: 반드시 사용자가 테스트 후 승인해야 함 (자의적 완료 판단 절대 금지)
> 5. 문서 양식 자의적 수정 금지
> 6. 파일 구조 암기: ls 명령어 남발 금지
> 7. nginx 설정 필수 기록
> 8. **작업 완료 후 무한 확인 루프 금지**: 작업 완료 시 간단히 보고만 하고 다음 작업 대기

### 🛡️ AI 자가 검증 프로세스 (재발 방지)
...
**강제 체크리스트:**
1. CRITICAL_CONFIG.md 전체 읽기
2. 유사 컴포넌트 3개 이상 패턴 참조
3. 백엔드-프론트엔드 응답 구조 일치 검증
4. 변수/상수 추가 시 grep으로 사용처 즉시 확인
5. 라우트 등록 후 즉시 동작 확인

자세한 내용: `CRITICAL_CONFIG.md` 규칙 0 참조
...
- **문제**:
  - **씬 누락**: 20초 영상(10개 씬) 생성 시 중간 씬 누락 (예: 씬 1-8, 10)
  - **누락 표시 없음**: 어떤 씬이 빠졌는지 알 수 없음
  - **Rate Limit 후행 처리**: 429 에러 발생 후 블록 (사전 예방 없음)
  - **Freepik 공식 Rate Limit**: 초당 50개 (5초), 평균 초당 10개 (2분)
- **근본 원인**:
  - 무지성 연속 요청 → 429 에러 → 블록
  - 실패한 씬 재시도 없음
  - 씬 순서 보장 안 됨
- **수정 내용**:
  1. **Rate Limiter 구현** (`src/utils/rateLimiter.js`):
     - **사전 예방적 속도 조절** (429 에러 발생 전에 대기)
     - 초당 최대 10개, Burst 최대 50개 (5초)
     - `waitForSlot()`: 슬롯 확보 전까지 자동 대기
     - 통계 조회: `getStats()`
  2. **apiHelpers.js - safeCallFreepik**:
     - Line 4: `import { freepikRateLimiter }` 추가
     - Line 332-334: **모든 Freepik 호출 전 `await freepikRateLimiter.waitForSlot()`**
     - 429 에러 사전 방지
  3. **storyboard-manual-inject.js - processManualStoryboard**:
     - Line 187: `sceneResults Map` 추가 (씬별 생성 결과 추적)
     - Line 227-282: **실패한 씬 재시도 로직** (최대 2회)
     - Line 284-298: **순서 보장 및 누락 씬 명시적 표시**
       - 성공한 씬: 정상 데이터
       - 실패한 씬: `title: "씬 N (생성 실패)"`, `error: "..."` 포함
- **개선 효과**:
  - ✅ Rate Limit 429 에러 **사전 방지**
  - ✅ 실패한 씬 **자동 재시도** (최대 2회)
  - ✅ 씬 순서 **완벽 보장** (1-8, 10 → 1-10으로 수정)
  - ✅ 누락 씬 **명확히 표시**
  - ✅ 동시 요청 시에도 안정적 처리
- **상태**: 코드 수정 완료, 테스트 대기

### 2026-01-07 03:05 - AdminPanel API 경로 버그 수정 및 응답 처리 버그 수정
- **파일**: `src/components/admin/AdminPanel.jsx`
- **근본 원인 (2단계 문제):**
  1. **API_BASE 상수 미선언** (1차 문제 - 이미 해결됨)
  2. **응답 필드 불일치** (진짜 근본 원인)
     - `api/api-keys.js`: `{success, message, keysUpdated}` 응답
     - `AdminPanel.jsx Line 529`: `data.restartResult.success` 접근 시도
     - **존재하지 않는 필드 접근** → `undefined.success` → TypeError
- **수정 내용**:
  1. Line 7-8: `const API_BASE = import.meta.env.VITE_API_BASE_URL || '/nexxii';` 추가
  2. 14개 fetch 호출 모두 `${API_BASE}/api/...`로 수정
  3. **Line 504-545: handleSaveKeys 함수 전면 수정**
     - `data.restartResult` 참조 완전 제거
     - `response.ok` 체크 추가
     - 에러 메시지에 `error.message` 포함
     - 성공 메시지 간소화
- **백엔드 vs 프론트엔드 불일치 해결:**
  - 백엔드: `{success: true, message: '...', keysUpdated: {gemini: 1, freepik: 1}}`
  - 프론트엔드: 이제 `data.success`, `data.keysUpdated`만 사용
- **재발 방지**:
  - API 응답 구조 변경 시 프론트엔드도 함께 수정
  - response.ok 체크 필수
- **상태**: 코드 수정 완료, 테스트 대기
...
- **문제**:
  - **필드명 불일치**: `checkUsageLimit()` 함수에서 `user.dailyLimit` 체크 (Line 223)
  - **실제 데이터**: `api/users.js`에서 `user.usageLimit` 사용
  - **결과**: 일일 사용 한도가 전혀 작동하지 않음
- **수정 내용**:
  - Line 216-243: `checkUsageLimit()` 함수 전면 수정
    - `user.dailyLimit` → `user.usageLimit` 변경
    - 무제한 사용자 처리 로직 추가 (`usageLimit === null` 체크)
    - 에러 메시지 개선: "관리자에게 문의하세요" 문구 추가
    - 리미트 초과 시 현재 사용량 표시: `(오늘: ${usageCount}/${usageLimit}회)`
    - 콘솔 에러 로깅 추가
- **검증**: 
  - `api/users.js`의 `checkUsageLimit()` 함수와 로직 일치 확인
  - 실제 DB 필드명(`usageLimit`) 사용 확인
- **상태**: 코드 수정 완료, 사용자 테스트 대기
