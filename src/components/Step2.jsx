...
// Z+ 추가: imagePrompt 확장 (원본 prompt 미변형)
function enrichPrompt(p){
  // 기존 prompt 객체(문자열만 있는 경우 포함) 보강
  const out = { ...p };
  if(!out.negative_prompt) out.negative_prompt = 'blurry, low quality, watermark, cartoon, distorted';
  if(!out.image) out.image = { size: 'widescreen_16_9' };
  if(!out.image.size) out.image.size='widescreen_16_9';
  if(!out.styling) out.styling = { style: 'photo' };
  if(!out.styling.style) out.styling.style='photo';
  if(!out.seed) out.seed = Math.floor(10000 + Math.random()*90000);
  if(!out.num_images) out.num_images = 1;
  return out;
}
...
const Step2 = ({ onNext, onPrev, formData, setStoryboard, setIsLoading, isLoading }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const fetchedStyles = (data.styles || []).map(style => {
        // imagePrompts 보강(Z+)
        style.images = [];
        style.imagePrompts = (style.imagePrompts||[]).map(enrichPrompt);
        return style;
      });
      ...
      log(`스토리보드 완료: 컨셉 ${fetchedStyles.length}개, 총 이미지 ${totalImages}개`);
      if(fetchedStyles.length===0){
        log('[Z+] 경고: styles 길이 0 → 자동 진행 중단 (Phase2 JSON만 온 케이스). 서버 파싱 로직 점검 필요.');
      }
      setPercent(15);

      // 2) 이미지 생성 (styles가 있을 때만)
      if(fetchedStyles.length>0){
      ...
      // 기존 코드: 여기서 onNext 자동 호출 → 조건부로 변경(Z+)
      if(fetchedStyles.length>0 && imagesTotalFromStyles(fetchedStyles)>0){
        onNext?.();
      } else {
        log('[Z+] 자동 이동하지 않음: styles 비어있음. 화면에서 문제 확인 후 재시도 해.');
      }
    } catch(e) {
  ...
  const generateOne = async (style, promptObj) => {
    try {
      ...
      if (!r.ok) {
        const txt = await r.text().catch(()=> '');
        log(`실패: [${style.style}] Scene ${promptObj.sceneNumber} ${r.status} ${txt?.slice?.(0,120)}`);
        setImgFail(f=>f+1);
        return;
      }
      const json = await r.json();
      let u = json.url;
      if(u && !/^data:|^https?:\/\//.test(u)){
        u = `${API_BASE}${u.startsWith('/')?u:`/${u}`}`;
      }
      style.images.push({
        id: `${style.concept_id}-${promptObj.sceneNumber}-${Math.random().toString(36).slice(2,7)}`,
        sceneNumber: promptObj.sceneNumber,
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className={`bg-white rounded shadow p-6 ${isLoading?'opacity-50 pointer-events-none':''}`}>
        ...
        <p className="text-gray-600 mb-6">
          Gemini 3회 호출(브리프→컨셉JSON/텍스트→6컨셉 멀티 스토리보드) 후 모든 컨셉 이미지를 Freepik으로 생성합니다.
          (styles 비어있으면 서버 파싱 실패 가능성 → 로그 확인)
        </p>
        ...
        {styles.length===0 && !isLoading && (
          <div className="mt-4 text-sm text-red-600">
            [Z+] styles 가 비어있음 → storyboard-init 응답이 Phase2 단일 JSON 형태였거나 파싱 실패.
            서버 로그의 컨셉 패턴( **1. 컨셉: …** ) 파싱 확장 로직 적용 여부 확인.
          </div>
        )}
      </div>
