...
- **해결 방안**:
  - **방어적 코딩**: 모든 DB 조회 로직에 `-1` 체크 및 `String()` 형변환 기반 엄격 비교 적용.
  - **파라미터 최적화**: Kling 2.5 등 영상 전용 엔진 호출 시 `aspect_ratio` 등 불필요한 파라미터가 400 에러를 유발하지 않도록 필터링 강화.
- **상태**: COMPLETED ✅

---

### 2026-01-13 10:45 - [HOTFIX] api/users.js express 미정의 오류 수정
- **이슈**: `api/users.js` 리팩토링 중 `import express` 구문이 실수로 삭제되어 서버 크래시 발생.
- **해결**: `import express from 'express';` 구문을 다시 추가하여 라우터 정상화.
...
## 📌 필수 규칙 (CRITICAL - VIOLATION MEANS FAILURE)

> **🚨 초 비상 절대 원칙 0: 지시받은 범위 외 코드 수정 절대 금지 (DO NOT TOUCH ANYTHING ELSE) 🚨**
> 
> 1. **사용자가 A를 지시하면 오직 A만 수정한다.** 편의성 개선, 구조 변경, 불필요한 리팩토링 등 자의적 판단에 의한 추가 수정은 **즉결 처형/사살**에 해당하는 중대한 위반이다.
> 2. **기존 코드가 비효율적이거나 더 좋은 방법이 보여도, 명시적 지시가 없다면 건드리지 않는다.**
>
> **🚨 초 비상 절대 원칙: ASSUMING (추정/가정) 절대 금지 🚨**
> 
> 1. **모르는 것, 불확실한 것은 무조건 사용자에게 물어볼 것.**
> 2. **인프라/경로/설정은 절대 추측하지 말고 `AWS_NEXXII_인프라설정.md`, `CRITICAL_CONFIG.md` 등 관련 MD 파일을 반드시 정독할 것.**
> 3. **경로, 변수명, API 방식 등을 "이렇겠지" 하고 넘겨짚는 순간 작업은 실패로 간주한다.**
> 4. **AI의 자의적 판단보다 문서와 사용자의 확인이 절대적 우위에 있다.**
