...
const Step3 = ({
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      for (const img of (selected.images || [])) {
        try {
          ...
          if (!r.ok) {
            ...
            log(`Task 실패 scene=${img.sceneNumber} ${r.status} ${txt.slice(0,80)}`);
            continue;
          }
      ...
      if (!newTasks.length) throw new Error('생성된 task 없음');
      setTasks(newTasks);
      setPolling(true);
    } catch (e) {
      setError(e.message);
    ...
    const poll = async () => {
      ...
      try {
        ...
        for (const t of tasks) {
          ...
          if (img?.videoUrl) completedCount++;
        }
        const p = Math.round((completedCount / tasks.length) * 100);
        setPercent(p);
